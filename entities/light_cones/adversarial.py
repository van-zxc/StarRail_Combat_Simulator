...
"""相抗 (Adversarial) — 3★ 巡猎光锥。

特效: 消灭敌方目标后，速度提高#1[i]%，持续#2[i]回合。
"""
...
class Adversarial(BaseLightCone):
    _default_id = "20014"
    _default_name = "相抗"
    ...
    def __init__(self, id: str = "", **kwargs):
        ...
        if self.effect is None:
            self.effect = AdversarialEffect(self.superimpose)


class AdversarialEffect(EquipmentEffect):
    _PARAMS = [0.10, 0.12, 0.14, 0.16, 0.18]
    _DURATION = 2
    _SOURCE = "LightCone_20014"
    ...
    def _on_kill(self, source: "Character") -> None:
        ...
        spd_pct = self._PARAMS[self.superimpose - 1]
        mod = StatModifier(
